...
const Following = () => {
    ...
    const followingPosts = posts.filter(post => followingIds.includes(post.author?._id))

    const postPerPage = 5; 
    const lastIndex = postPerPage * trendingPage;
    const firstIndex = lastIndex - postPerPage;
    const currentPosts = followingPosts.slice(firstIndex, lastIndex)

    const totalPages = Math.ceil(followingPosts.length / postPerPage)
